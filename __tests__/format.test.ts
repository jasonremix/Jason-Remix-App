...
describe('countdowns', () => {
  ...
  it('counts down in the largest sensible unit', () => {
    expect(formatTimeRemaining('2026-08-01T12:30:00Z', now)).toBe('30 MIN LEFT');
    expect(formatTimeRemaining('2026-08-01T18:00:00Z', now)).toBe('6 HOURS LEFT');
    expect(formatTimeRemaining('2026-08-01T13:00:00Z', now)).toBe('1 HOUR LEFT');
    expect(formatTimeRemaining('2026-08-06T12:00:00Z', now)).toBe('5 DAYS LEFT');
  });

  it('reports a past deadline as ended', () => {
    expect(formatTimeRemaining('2026-07-31T12:00:00Z', now)).toBe('ENDED');
  });

  it('describes a mission cooldown', () => {
    expect(formatCooldown('2026-08-01T18:30:00Z', now)).toBe('READY IN 6H 30M');
    expect(formatCooldown('2026-08-01T12:45:00Z', now)).toBe('READY IN 45M');
    expect(formatCooldown('2026-08-01T11:00:00Z', now)).toBe('READY');
  });
});
